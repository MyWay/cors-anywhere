...
function getHandler(options, proxy) {
    ...
    return function (req, res) {
        ...
        var targetURL = req.url.slice(1); // Remove the leading slash
        try {
            var targetDomain = new URL(targetURL).hostname;
            // Now check whitelist/blacklist against targetDomain instead of origin
            if (corsAnywhere.originBlacklist.indexOf(targetDomain) >= 0) {
            ...
            if (
                ...
                corsAnywhere.originWhitelist.indexOf(targetDomain) === -1
            ) {
                res.writeHead(403, 'Forbidden', cors_headers);
                res.end(
                    'The domain "' +
